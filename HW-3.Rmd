...
training <- q1_profiles[sample(1:nrow(q1_profiles), 2997, replace=FALSE),] %>% 
  select(id, age, body_type, height, income, job, orientation, sex, is_female)
...
bitcoin <- bitcoin %>%
  ...
  arrange(Date) %>%
  mutate(Percent_Change = Price / lag(Price, 1) - 1)   # calc percentage change
gold <- gold %>%
  filter(Date %within% common_range) %>%
  mutate(Percent_Change = Price / lag(Price, 1) - 1)   # calc percentage change
...
ggplot(bitcoin, aes(x = Date, y = Price)) +
  geom_line(color = "dark blue") +                # Line for Bitcoin
  geom_line(data = gold, color = "dark red") +    # Line for Gold
  ylim(0, 1900) +
  geom_text(data = bitcoin[bitcoin$Date == end,],
            aes(label = "Bitcoin"), hjust = 0.7, vjust = -2)  +
  geom_text(data = bitcoin[bitcoin$Date == end,],
            aes(label = "Gold"), hjust = 0.7, vjust = -14) +
  labs(y = "Price (USD)", title = "Volatility in Bitcoin & Gold Relative to the US Dollar")

# Percentage Change
ggplot(bitcoin, aes(x = Date, y = Percent_Change)) +
  geom_bar(stat = "identity", color = "dark blue") +               # For bitcoin
  geom_bar(data = gold, stat = "identity", color = "dark red") +   # For gold
  ylim(-0.4, 0.4) +
